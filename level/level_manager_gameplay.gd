...
func _init(game_state: GameState, controls: Node) -> void:
	...
	_state_controller.level_complete.connect(func(): level_complete.emit())
...
func _on_dialogue_finished(flag_id: StringName) -> void:
	if not _game_state or not _game_state.task_controller: return
...
func apply_level_if_available() -> void:
	...
	_handle_build_result(builder.build(_level_resource, _game_state.map_controller.get_terrain_map()))
...
func _connect_morale_panel_signals() -> void:
	...
	if is_instance_valid(morale_panel):
		if not morale_panel.player_retreat_triggered.is_connected(_state_controller.handle_player_defeat.bind("GAME OVER! Morale Broken!")):
			morale_panel.player_retreat_triggered.connect(_state_controller.handle_player_defeat.bind("GAME OVER! Morale Broken!"))
...
func set_level_and_rebuild(level: Level) -> void:
	_level_resource = level
	if not _game_state or not is_instance_valid(_game_state.task_controller): return
...
func on_task_failed() -> void:
	_state_controller.handle_player_defeat("Enemy secured the objectives! Retreat!")
