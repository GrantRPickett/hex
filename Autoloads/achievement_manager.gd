		...
		if res is Resource and is_instance_valid(res):
			...
			if achievement_instance is Resource:
				if not achievements.has(ach_id):
					...
					GameLogger.warning(GameLogger.Category.ACHIEVEMENTS, "Duplicate achievement ID found: '%s'" % ach_id)
...
func unlock_achievement(achievement_id: String) -> bool:
	...
	if achievement is Resource:
		if not achievement.get("unlocked"):
			...
			GameLogger.info(GameLogger.Category.ACHIEVEMENTS, "AchievementManager: Unlocked achievement: %s" % (title if title else achievement_id))
			return true
		else:
			GameLogger.info(GameLogger.Category.ACHIEVEMENTS, "AchievementManager: Achievement '%s' already unlocked." % achievement_id)
	else:
		GameLogger.warning(GameLogger.Category.ACHIEVEMENTS, "AchievementManager: Attempted to unlock non-existent achievement: %s" % achievement_id)
	return false
	...
	if data.has("unlocked_achievements"):
		...
		for achievement_id: String in unlocked_ids:
			...
			if achievement is Resource:
				...
				GameLogger.warning(GameLogger.Category.ACHIEVEMENTS, "AchievementManager: Saved data refers to non-existent achievement ID: %s" % achievement_id)
