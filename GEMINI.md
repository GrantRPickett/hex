...
- **Diagnostics:** If a test fails with a runtime error (e.g., "previously freed"), use `grep` to identify the exact line in the source, add `print_debug()` for state tracking, and verify the output in the `reports/*.log` files.
- **Execution:** Prefer running individual test suites using `powershell -File scripts/run_tests.ps1 -Test res://path/to/test.gd` to minimize noise.
