...
if __name__ == "__main__":
    ...
    with open('long_funcs_filtered.txt', 'w', encoding='utf-8') as f:
        for func in long_funcs[:30]: # Top 30 non-addon functions
            f.write(f"File: {func['file']}\n")
            f.write(f"  Function: {func['name']}\n")
            f.write(f"  Length: {func['length']} lines\n")
            f.write(f"  Lines: {func['start']}-{func['end']}\n\n")

    print("Filtered long functions report generated in long_funcs_filtered.txt")
