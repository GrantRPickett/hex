...
func test_weather_checkpoint_restores_pressures_and_channeling() -> void:
	...
	var initial_manager: UnitManager = auto_free(UnitManager.new())
	...
	var snapshot: Dictionary = weather.create_memento(initial_manager)
	var restored_manager: UnitManager = auto_free(UnitManager.new())
	var restored_a: Unit = auto_free(Unit.new())
	var restored_b: Unit = auto_free(Unit.new())
