...
func test_action_points_component_tracks_turn_state() -> void:
	...
	assert_bool(component.movement.has_move_available()).is_true()
	assert_int(component.movement.get_remaining_movement_points()).is_equal(5)
	component.movement.consume_move(3)
	assert_int(component.movement.get_remaining_movement_points()).is_equal(2)
	component.adjust_remaining_movement(-1)
	assert_int(component.movement.get_remaining_movement_points()).is_equal(1)
	component.block_movement_this_turn()
	assert_bool(component.movement.has_move_available()).is_false()
	component.refresh_for_new_round()
	component.res.consume_action()
	assert_bool(component.res.has_action_available()).is_false()
...
func test_inventory_component_applies_item_modifiers() -> void:
	...
	var inventory: UnitInventory = component.inv.get_inventory()
	var item: InventoryItem = InventoryItem.new()
	item.attribute_modifiers = {"grit": 1}
	assert_bool(component.inv.equip_item(item)).is_true()
	assert_int(attributes.get_attribute("grit")).is_equal(7)
	assert_bool(component.inv.unequip_item(item)).is_true()
	component.cleanup()
...
func test_set_start_of_turn_grid_coord_updates_anchor() -> void:
	...
	assert_that(behavior.movement.get_start_of_turn_grid_coord()).is_equal(anchor)
