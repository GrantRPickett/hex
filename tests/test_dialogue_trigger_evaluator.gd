...
func test_is_trigger_available() -> void:
	var evaluator = DialogueTriggerEvaluator.new()
	...
	assert_bool(evaluator.is_trigger_available(trigger, null, null)).is_true()

	trigger.mark_seen()
	assert_bool(evaluator.is_trigger_available(trigger, null, null)).is_false()
...
func test_collect_partner_and_initiator_indices() -> void:
	...
	evaluator.setup(unit_manager)

	# Test methods collect_partner_indices and collect_initiator_indices
	assert_array(evaluator.collect_partner_indices(trigger)).is_empty()
	assert_array(evaluator.collect_initiator_indices(trigger)).is_empty()
