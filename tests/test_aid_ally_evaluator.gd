...
func test_evaluate_scores_zero_if_ally_has_no_actions() -> void:
	do_return({"enemies": [], "allies": [ally], "neutrals": []}).on(unit.query).get_near_units_categorized(any_float())
...
func test_evaluate_lowers_score_if_ally_not_near_enemy() -> void:
	do_return({"enemies": [], "allies": [ally], "neutrals": []}).on(unit.query).get_near_units_categorized(any_float())
	do_return({"enemies": [], "allies": [], "neutrals": []}).on(ally.query).get_near_units_categorized(any_float())
...
func test_evaluate_full_score_if_ally_near_enemy() -> void:
	do_return({"enemies": [], "allies": [ally], "neutrals": []}).on(unit.query).get_near_units_categorized(any_float())
	do_return({"enemies": [enemy], "allies": [], "neutrals": []}).on(ally.query).get_near_units_categorized(any_float())
