...
func test_get_available_actions_includes_wait_when_turn_enabled() -> void:
	...
	var actions = _UnitActionManager.get_available_actions(unit, null, manager)
	var has_wait := false
	for action in actions:
		if action.get("action_id", "") == "hud.action_wait_end_turn":
			has_wait = true
			break
...
func test_move_and_interact_action_generates_attack_option() -> void:
	...
	_UnitActionManager._append_move_and_interact_actions(actions, unit, null, manager, reachable_lookup, TileSet.TILE_OFFSET_AXIS_VERTICAL)
	var found := false
	for action in actions:
...
func test_move_and_attack_uses_zero_move_when_tentative_origin_is_adjacent() -> void:
	...
	var actions: Array[Dictionary] = []
	_UnitActionManager._append_move_and_interact_actions(actions, unit, terrain, manager, reach_state.lookup, TileSet.TILE_OFFSET_AXIS_VERTICAL)
	var attack_action := {}
	for action in actions:
		if action.get("interact_action_type", "") == "attack":
			attack_action = action
...
func test_resolve_move_cost_respects_remaining_move() -> void:
	...
	assert_int(_UnitActionManager._resolve_move_cost(reachable_lookup, Vector2i(1, 0), 2)).is_equal(1)
	assert_int(_UnitActionManager._resolve_move_cost(reachable_lookup, Vector2i(2, 0), 2)).is_equal(-1)
	assert_int(_UnitActionManager._resolve_move_cost(reachable_lookup, Vector2i(3, 0), 2)).is_equal(-1)
...
func test_build_move_and_interact_action_merges_extra_fields() -> void:
	...
	var action := _UnitActionManager._build_move_and_interact_action(
		Vector2i(3, 1),
		"explore",
		2,
		1,
...
func test_move_and_loot_action_skipped_when_path_blocked() -> void:
	...
	var actions: Array[Dictionary] = []
	_UnitActionManager._append_move_and_interact_actions(actions, unit, terrain_map, manager, reachable_lookup, TileSet.TILE_OFFSET_AXIS_VERTICAL)
	assert_array(actions).is_empty()
	var unit_index := manager.get_unit_index(unit)
	assert_bool(_UnitActionManager._has_unblocked_path(unit, terrain_map, manager, unit_index, Vector2i(3, 1), unit.movement_points)).is_false()
...
func test_resolve_move_origin_uses_committed_coord_for_tentative_move() -> void:
	...
	var origin = _UnitActionManager._resolve_move_origin(unit, manager, manager.get_unit_index(unit))
