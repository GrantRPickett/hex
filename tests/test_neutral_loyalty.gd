...
func test_neutral_handles_attack_from_player() -> void:
	...
	assert_int(neutral.loyalty.neutral_loyalty).is_equal(Unit.Faction.ENEMY)
...
func test_neutral_persuasion_changes_loyalty() -> void:
	...
	assert_int(neutral.loyalty.neutral_loyalty).is_equal(Unit.Faction.PLAYER)

func test_reset_neutral_loyalty_clears_alignment() -> void:
	var neutral := _make_unit(Unit.Faction.NEUTRAL)
	neutral.neutral_can_be_persuaded = true
	neutral.loyalty.apply_persuasion(Unit.Faction.ENEMY)
	neutral.loyalty.reset_neutral_loyalty()
	assert_int(neutral.loyalty.neutral_loyalty).is_equal(Unit.Faction.NEUTRAL)
...
func test_rally_spreads_loyalty_to_targets() -> void:
	...
	leader.loyalty.set_neutral_loyalty(Unit.Faction.PLAYER, true, [follower])
