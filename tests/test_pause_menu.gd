...
var _audio_bus_controller: Stubs.FakeAudioBusController
var _game_config: Stubs.FakeGameConfig
...
var _dialogic: Stubs.FakeDialogic
...
func before_test() -> void:
	...
	_audio_bus_controller = Stubs.FakeAudioBusController.new()
	...
	_game_config = Stubs.FakeGameConfig.new()
	...
	_dialogic = Stubs.FakeDialogic.new()
...
func test_orientation_selection_updates_display_settings_and_config() -> void:
	...
	assert_that(_display_settings.orientation).is_equal(1) # PORTRAIT (assumes 1 = PORTRAIT)
	...
	assert_that(resolution_option.get_item_count()).is_equal(_display_settings.portrait.size())

func test_resolution_selection_updates_display_settings_and_config() -> void:
	var scene: Control = _runner.scene() as Control
	scene._on_resolution_selected(1)
	assert_that(_display_settings.get_current_resolution_index()).is_equal(1)
	assert_that(_game_config.get_value("display/resolution")).is_equal(_display_settings.get_current_resolution())
...
func test_auto_advance_toggle_updates_dialogic_and_config() -> void:
	...
	assert_bool(_dialogic.Inputs.auto_advance.enabled_until_user_input).is_true()
	assert_bool(_dialogic.Inputs.auto_advance.enabled_forced).is_true()
...
func test_auto_advance_speed_slider_updates_settings_and_config() -> void:
	...
	assert_that(_dialogic.Settings.autoadvance_delay_modifier).is_equal(1.2)
...
func test_text_speed_slider_updates_settings_and_config() -> void:
	...
	assert_that(_dialogic.Settings.text_speed).is_equal(0.6)
