...
func test_wait_action_executes_wait_command() -> void:
	...
	var wait_action := PlayerAction.new(GameConstants.ActionType.WAIT)
	await _hud.on_action_selected(wait_action)
	assert_array(_get_command_ids()).is_equal([GameConstants.Commands.CommandID.WAIT])
	assert_signal(_hud).is_emitted("action_executed", [GameConstants.ActionType.WAIT])
...
func test_attack_action_routes_payload_to_input_controller() -> void:
	...
	var attack_action := PlayerAction.new(GameConstants.ActionType.ATTACK)
	...
	assert_signal(_hud).is_emitted("action_executed", [GameConstants.ActionType.ATTACK])
