...
func test_collect_all_loot_items_removes_entries() -> void:
	...
	assert_str(collected[0].get_item_name()).is_equal("Coin")
	assert_int(loot_manager.get_loot_count()).is_equal(0)

func test_collect_routing_pool() -> void:
	var loot_manager: LootManager = auto_free(LootManager.new())
	# Mock routing pool with InventoryItems since we've typed it
	var item: InventoryItem = InventoryItem.new()
	loot_manager._routing_pool = [item]
