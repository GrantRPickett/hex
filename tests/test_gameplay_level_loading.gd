...
func test_gameplay_applies_level_manager_selection() -> void:
    ...
    var level := load(LEVEL2_PATH) as Resource

    assert_that(scene.level_resource).is_not_null()
    assert_that(scene.player_coord).is_equal(level.get("player1_start"))
    assert_that(scene.player2_coord).is_equal(level.get("player2_start"))
    assert_that(scene.goal_coord).is_equal(level.get("goal_coord"))
    assert_that(scene.goal2_coord).is_equal(level.get("goal2_coord"))
    assert_that(scene._grid_width).is_equal(int(level.get("grid_width")))

    var axis := int(level.get("hex_offset_axis"))
    assert_that(scene._grid.tile_set.tile_offset_axis).is_equal(axis)
    assert_that(scene._use_dual_goals).is_equal(bool(level.get("require_units_match_goals")))
