...
func test_set_reachable_info_uses_coord_lookup() -> void:
	var target: Stubs.FakeUnit = auto_free(Stubs.FakeUnit.new())
	target.set_grid_location(Vector2i(4, 4))
	var action: PlayerAction = PlayerAction.new(GameConstants.ActionType.CONVINCE)
	var lookup := {
	...
	assert_vector(info.get("coord", GameConstants.INVALID_COORD)).is_equal(target.get_grid_location())
...
func test_set_reachable_info_accepts_target_keyed_lookup() -> void:
	...
	assert_bool(action.target_move_data.has(target)).is_true()
	var info: Dictionary = action.target_move_data[target]
	assert_vector(info.get("coord", GameConstants.INVALID_COORD)).is_equal(Vector2i(6, 2))
...
func test__clone_move_info_normalizes_inputs() -> void:
	...
	assert_vector(info.get("coord", GameConstants.INVALID_COORD)).is_equal(Vector2i(2, 2))
