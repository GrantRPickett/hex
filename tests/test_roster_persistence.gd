...
func test_unit_to_entry_captures_unit_metadata() -> void:
	var unit: Node = auto_free(GenericUnitScene.instantiate() as Unit)
...
func test_entry_to_scene_restores_memento() -> void:
	...
	var restored_unit: Node = auto_free(scene.instantiate() as Unit)
...
func test_scene_to_entry_preserves_fallback_scene() -> void:
	var temp_unit: Unit = GenericUnitScene.instantiate() as Unit
	...
	var entry: Dictionary = RosterPersistence.scene_to_entry(packed)
