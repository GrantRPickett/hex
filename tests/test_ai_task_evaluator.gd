...
func test_evaluate_returns_work_on_task_if_at_location() -> void:
	...
	assert_object(actions[0].target_object).is_same(task_obj)
...
func test_evaluate_returns_move_to_task_for_distant_task() -> void:
	...
	var actions = evaluator.evaluate(unit, context)
	assert_int(actions.size()).is_equal(1)
	assert_int(actions[0].type).is_equal(GameConstants.AI.ACTION_MOVE_TO_EXPLORE)
	assert_object(actions[0].target_object).is_same(task)
