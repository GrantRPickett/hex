...
func test_multi_step_progression() -> void:
	...
	_location_manager.setup(coords, [location], null)
...
func test_faction_independence() -> void:
	...
	var single_step: Array[locationStep] = []
	...
	_location_manager.setup([Vector2i(0, 0)], [location], null)
...
func test_remaining_location_titles_reflect_unfinished_required_locations() -> void:
	var def_a = locationDefinition.new()
	def_a.title = "locationA"
	var step_a := locationStep.new()
	step_a.required_amount = 1
	step_a.required_attribute = "grit"
	def_a.steps = [step_a]
	var def_b = locationDefinition.new()
	def_b.title = "locationB"
	var step_b := locationStep.new()
	step_b.required_amount = 1
	step_b.required_attribute = "grit"
	def_b.steps = [step_b]
	var location_a = auto_free(location.new())
	location_a.definition = def_a
	var location_b = auto_free(location.new())
	location_b.definition = def_b
	_location_manager.setup([Vector2i(0, 0), Vector2i(1, 0)], [location_a, location_b], null)
	var unit = auto_free(Unit.new())
	unit.faction = Unit.Faction.PLAYER
	var remaining := _location_manager.get_remaining_location_titles()
	assert_int(remaining.size()).is_equal(2)
	assert_str(remaining[0]).is_equal("locationA")
	assert_str(remaining[1]).is_equal("locationB")
	_location_manager.apply_progress(0, unit)
	remaining = _location_manager.get_remaining_location_titles()
	assert_int(remaining.size()).is_equal(1)
	assert_str(remaining[0]).is_equal("locationB")
