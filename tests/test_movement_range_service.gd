...
func _create_plain_terrain() -> TerrainMap:
	...
	terrain.load_from_rows(["GGGG", "GGGG"], 4, 2)
	return terrain

func test_calculate_reports_move_spaces() -> void:
	var setup: Vector2i = _create_unit_setup(Vector2i(1, 1))
	var terrain: TerrainMap = _create_plain_terrain()
	var state: ReachableState = MovementRangeService.calculate_reachable_state(setup.unit, terrain, setup.manager)
...
func test_calculate_uses_tentative_action_origin() -> void:
	...
	var state: ReachableState = MovementRangeService.calculate_reachable_state(setup.unit, null, setup.manager)
