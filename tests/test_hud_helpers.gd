...
class TestInputController extends InputController:
	...
	func _execute_command(command_name: String, payload = null) -> CommandResult:
		...
		if command_name == "confirm_move" and tentative_stub:
			tentative_stub.pending = false
		return CommandResult.success()

class TentativeMovement extends RefCounted:
	var pending := true
	func has_tentative_move() -> bool:
		return pending

class TentativeUnit extends Unit:
	var movement_stub := TentativeMovement.new()
	func _init():
		super._init()
...
func test_command_success_helper_detects_status() -> void:
	assert_bool(_hud._command_success(CommandResult.failed())).is_false()
	assert_bool(_hud._command_success(CommandResult.success())).is_true()

func test_run_input_command_returns_null_without_controller() -> void:
	_hud._input_controller = null
	assert_object(_hud._run_input_command("test")).is_null()
	_hud._input_controller = _controller
	assert_object(_hud._run_input_command("test_again")).is_not_null()
...
func test_execute_attack_and_support_commands_route_payloads() -> void:
	var attack_action := {"target": _target, "attribute_index": 1}
	assert_object(_hud._execute_attack_command(attack_action)).is_not_null()
	assert_str(_controller.last_command).is_equal(GameConstants.Commands.ATTACK)
	assert_object(_hud._execute_attack_payload(1, 0)).is_not_null()
	assert_object(_hud._execute_aid_command({"target": _target})).is_not_null()
	assert_str(_controller.last_command).is_equal(GameConstants.Commands.AID)
	assert_object(_hud._execute_convince_command({"target": _target})).is_not_null()
	assert_object(_hud._execute_convince_payload(1)).is_not_null()

func test_execute_loot_skill_and_talk_commands() -> void:
	assert_object(_hud._execute_loot_command({})).is_not_null()
	assert_object(_hud._execute_loot_payload(Vector2i(2, 2))).is_not_null()
	assert_object(_hud._execute_skill_command({"skill": "Focus"})).is_not_null()
	var talk_action := {"target_index": 1, "dialogue_id": "dlg_1"}
	assert_object(_hud._execute_talk_command(talk_action)).is_not_null()
