...
static func direction_map_for_x(x: int, even: Dictionary, odd: Dictionary) -> Dictionary:
    return even if (x & 1) == 0 else odd

static func analog_vectors_for_x(x: int, even: Dictionary, odd: Dictionary) -> Dictionary:
    return even if (x & 1) == 0 else odd

static func cache_analog_vectors(grid: TileMapLayer, dirs_even: Dictionary, dirs_odd: Dictionary) -> Dictionary:
    var out := {"even": {}, "odd": {}}
    ...
    for action in dirs_even.keys():
        ...
        out["even"][action] = v
    var odd_origin_coord := Vector2i(1, 0)
