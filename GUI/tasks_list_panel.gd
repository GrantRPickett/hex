...
func _update_display() -> void:
	...
	for faction_group in _last_tasks_data:
		...
		for task_data in display_tasks:
			...
			if task_item.has_method("update_task") and task_item is TaskListItem:
				...
				# Connect completion_requested signal from TaskListItem
				var tli = task_item as TaskListItem
				GameLogger.debug(GameLogger.Category.UI, "[TasksListPanel] Connecting task item: ", task_data.get("id", ""))
				tli.completion_requested.connect(func(id): task_completion_requested.emit(id))
