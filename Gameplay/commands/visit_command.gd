...
func execute(context: GameCommandContext, payload: Dictionary = {}) -> CommandResult:
	...
	var target_info := TaskResolutionService.resolve_task_and_target(context, payload, GameConstants.TaskEvents.VISIT, faction)
	var task: Task = target_info.task
	var target: Target = target_info.target

	if task == null:
		return CommandResult.invalid_payload("Task required for visit")
	if not is_instance_valid(target):
		return CommandResult.invalid_payload("Target required for visit")
