...
func snap_to_grid(node: Node2D) -> Vector2i:
	if not is_instance_valid(_grid): return GameConstants.INVALID_COORD
	var coord: Vector2i = _grid.local_to_map(_grid.to_local(node.global_position))
...
func is_unit_at(coord: Vector2i, ignore_unit: Unit = null) -> bool:
	...
	var unit: Unit = _unit_manager.get_unit_at_coord(coord)
...
func get_nearest_empty_coord(requested_coord: Vector2i, max_radius: int = 5) -> Vector2i:
	...
	var axis: int = TileSet.TILE_OFFSET_AXIS_VERTICAL
	var units: Array[Unit]= _unit_manager.get_units()
	...
	while not queue.is_empty():
		...
		for i in range(layer_size):
			...
			var offsets: Array = HexLib.get_neighbor_offsets(current, axis)
