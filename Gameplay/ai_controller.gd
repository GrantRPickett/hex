...
func _execute_unit_interaction(ai_unit: Unit, best_action: AIAction) -> bool:
	...
	match best_action.type:
		...
		ACTION_MOVE_TO_ENEMY, ACTION_MOVE_TO_TASK, ACTION_MOVE_TO_LOOT, ACTION_MOVE_TO_CENTER, ACTION_MOVE_TO_TALK:
			return false
		_:
...
func _find_task_actions(ai_unit: Unit, _start_pos: Vector2i, terrain_map, actions: Array[AIAction], threatened_hexes: Dictionary = {}) -> void:
	...
	for i in range(_task_manager.get_location_count()):
		...
		if _unit_manager.is_occupied(task_coord):
			continue
