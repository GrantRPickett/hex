		...
		if trigger.matches_initiator(unit):
			...
			if partners.is_empty():
				if can_proceed_without_partner(trigger):
					actions.append(build_dialogue_action(trigger, unit_index, unit_index, trigger.get_action_label(""), unit))
			else:
				for partner in partners:
					...
					actions.append(build_dialogue_action(trigger, unit_index, partner_index, label, partner))
			...
			for initiator in initiators:
				...
				actions.append(build_dialogue_action(trigger, initiator_idx, unit_index, label, unit))
...
func build_dialogue_action(trigger: DialogueTrigger, initiator_index: int, partner_index: int, label: String, p_target: Object = null) -> PlayerAction:
	var action: PlayerAction = PlayerAction.new(GameConstants.ActionType.TALK)
	action.label = label
	action.dialogue_id = String(trigger.get_dialogue_id())
	action.initiator_index = initiator_index
	action.target_index = partner_index
	action.target_object = p_target
