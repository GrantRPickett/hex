...
func add_topic(topic: JournalTopic):
	if not topics.has(topic.id):
		...
		if not sections.has(topic.section_id):
			var new_section: JournalSection = JournalSection.new(topic.section_id, topic.section_id.capitalize())
...
func add_entry(entry: JournalEntry):
	if not entries.has(entry.id):
		...
		if not topics.has(entry.topic_id):
			...
			var new_topic: JournalTopic = JournalTopic.new(entry.topic_id, entry.topic_id.capitalize(), entry.section_id if not entry.section_id.is_empty() else "objectives") # Use entry.section_id or default
			add_topic(new_topic)
...
func replace_entry(entry: JournalEntry):
	if entries.has(entry.id):
		...
		if not topics.has(entry.topic_id):
			var new_topic: JournalTopic = JournalTopic.new(entry.topic_id, entry.topic_id.capitalize(), entry.section_id if not entry.section_id.is_empty() else "objectives")
			add_topic(new_topic)
	...
	if sections.has(section_id):
		...
		for topic_id in section.topic_ids:
			...
			if topic:
				...
				var has_unlocked_entry: bool = false
