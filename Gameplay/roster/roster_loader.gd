...
func load_player_roster(provided_roster: PlayerRoster, save_manager: Node, fallback_path: String = DEFAULT_PLAYER_ROSTER_PATH) -> PlayerRoster:
	if provided_roster:
		if not provided_roster.units.is_empty():
			GameLogger.info(GameLogger.Category.SYSTEM, LOG_PREFIX, " Using provided player roster with ", provided_roster.units.size(), " units.")
			return provided_roster
		GameLogger.info(GameLogger.Category.SYSTEM, LOG_PREFIX, " Provided player roster is empty. Falling back to defaults.")
	...
	GameLogger.info(GameLogger.Category.SYSTEM, LOG_PREFIX, " Returning new empty PlayerRoster.")
...
func _load_saved_player_roster(save_manager: Node) -> PlayerRoster:
	if save_manager and save_manager.has_method("has_saved_roster") and save_manager.has_saved_roster():
		...
		if saved and not saved.units.is_empty():
			GameLogger.info(GameLogger.Category.SYSTEM, LOG_PREFIX, " Loaded saved player roster with ", saved.units.size(), " units.")
			return saved
	return null

func _load_player_roster_resource(path: String) -> PlayerRoster:
	if not path.is_empty() and ResourceLoader.exists(path):
		GameLogger.info(GameLogger.Category.SYSTEM, LOG_PREFIX, " Loading default player roster from ", path)
		...
		if loaded_roster_data is PlayerRoster:
			...
			GameLogger.info(GameLogger.Category.SYSTEM, LOG_PREFIX, " Default player roster loaded successfully with ", roster.units.size(), " units.")
			return roster
		GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Error: ", path, " is not a PlayerRoster resource. It is: ", loaded_roster_data)
	elif not path.is_empty():
		GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Error: Default player roster not found at ", path)
...
func _load_unit_roster(provided_roster: UnitRoster, fallback_path: String, roster_class: GDScript, roster_label: String, resource_label: String, warn_on_empty := true) -> UnitRoster:
	...
	if provided_roster and provided_roster.units.is_empty():
		if warn_on_empty:
			GameLogger.info(GameLogger.Category.SYSTEM, LOG_PREFIX, " Provided %s roster is empty. Falling back to defaults." % roster_label)
	...
	if ResourceLoader.exists(fallback_path):
		...
		if loaded is UnitRoster and loaded.get_script() == roster_class:
			return loaded
		GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Error: ", fallback_path, " is not a ", roster_label, ".")
	else:
		GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Error: Default ", resource_label, " not found at ", fallback_path)
...
func _populate_roster_from_resource(target_roster: UnitRoster, path: String, roster_class: GDScript, roster_label: String, resource_label: String) -> void:
	...
	if not ResourceLoader.exists(path):
		GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Warning: ", resource_label, " not found at ", path)
		return
	...
	if loaded_roster_data is UnitRoster and loaded_roster_data.get_script() == roster_class:
		...
		for scene in loaded_roster_data.units:
			if scene is PackedScene:
				...
				GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Warning: Element in ", path, " is not a PackedScene. Skipping.")
	elif loaded_roster_data is UnitRoster:
		GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Warning: ", path, " is not a ", roster_label, " resource. Using empty roster.")
	else:
		GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Warning: Resource at ", path, " is not a UnitRoster. Using empty roster.")
...
func _build_core_player_roster() -> PlayerRoster:
	...
	if roster.units.is_empty():
		GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Warning: No core character scenes found in ", CORE_PLAYER_ROSTER_DIR)
		return null
...
func _instantiate_core_units(roster: PlayerRoster) -> void:
	...
	if dir == null:
		GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Warning: Could not open core roster directory ", CORE_PLAYER_ROSTER_DIR)
		return
	...
	for file_name in files:
		...
		if packed is PackedScene:
			...
			if instance is Unit:
				...
				GameLogger.error(GameLogger.Category.SYSTEM, LOG_PREFIX, " Warning: Scene at ", scene_path, " is not a Unit.")
			if instance is Node:
				instance.queue_free()
