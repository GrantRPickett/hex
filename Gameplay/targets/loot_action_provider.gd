...
func append_loot_action(actions: Array[PlayerAction], unit: Unit, reach: ReachableState) -> void:
	...
	_add_categorized_loot_actions(actions, split_loot, reach.lookup, target_to_task, unit)

func _add_categorized_loot_actions(actions: Array[PlayerAction], split_loot: Dictionary, reachable_lookup: Dictionary, target_to_task: Dictionary, actor: Unit) -> void:
	if split_loot.immediate_trapped or not split_loot.reachable_trapped.is_empty():
		_add_loot_action(actions, split_loot.immediate_trapped, split_loot.reachable_trapped, reachable_lookup, GameConstants.ActionType.TRAPPED, GameConstants.ActionIds.ITEM_OPPOSED, target_to_task, actor)

	if split_loot.immediate_gather or not split_loot.reachable_gather.is_empty():
		_add_loot_action(actions, split_loot.immediate_gather, split_loot.reachable_gather, reachable_lookup, GameConstants.ActionType.GATHER, GameConstants.ActionIds.ITEM_UNOPPOSED, target_to_task, actor)

func _add_loot_action(actions: Array[PlayerAction], immediate_loot: Loot, reachable_loot: Array, reachable_lookup: Dictionary, action_type: GameConstants.ActionType, action_id: String, target_to_task: Dictionary = {}, actor: Unit = null) -> void:
	...
	if loot_immediate_count > 0 or loot_reachable_count > 0:
		...
		loot_action.actor = actor
		loot_action.ui_label_params = {"near": loot_immediate_count, "far": loot_reachable_count}
		...
		if loot_immediate_count > 0:
			...
			loot_action.targets.append(immediate_loot)
		...
		if loot_reachable_count > 0:
			...
			for loot in reachable_loot:
				loot_action.reachable_targets.append(loot)
